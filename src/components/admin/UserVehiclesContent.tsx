...
interface UserVehiclesContentProps {
  userId: number
  customerId: number | null
  onRefresh?: () => void
}

export default function UserVehiclesContent({ userId, customerId, onRefresh }: UserVehiclesContentProps) {
...
function AddEditVehicleModal({ customerId, vehicle, onClose, onSuccess }: AddEditVehicleModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!customerId) {
      toast.error('Cần customerId để thêm/sửa xe')
      return
    }
    try {
      setSubmitting(true)
